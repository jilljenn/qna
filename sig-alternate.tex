...
\While{\textsc{TerminationRule} is not satisfied}
	...
	\State \Call{EstimateParameters}{}
\EndWhile
\State $p \gets$ \Call{PredictPerformance}{$parameters$}
\State \Call{Evaluate}{$p$}
\EndProcedure
\end{algorithmic}
\end{algorithm}

Our framework relies on the following subroutines:

\begin{itemize}
\item \textsc{TrainingStep} : Train the model with a subset of student data.
\item \textsc{NextItem} : Choose the best question to ask according to a certain criterion.
\item \textsc{EstimateParameters} : Estimate parameters according to the answer.
\item \textsc{PredictPerformance} : Predict performance on the whole test with respect to the parameters of the model.
\item \textsc{Evaluate} : Evaluate the performance prediction according to the true response vector.
\end{itemize}
...
Using a variant of the EM algorithm, we determine from the training set all the parameters of the model, i.e. difficulty $d_j$ and discrimination $\delta_j$ of each question and ability $\theta_i$ of each training student.
...
We estimate $\theta$ via Bayesian updates according to her answer.

\subsubsection{Performance Prediction}

We compute her probability of answering correctly to the remaining questions in the bank and error.

\subsection{Cognitive Diagnosis Model Design}

\subsubsection{Training Step}

We determine the q-matrix $Q$, the slipping and guessing parameters according to our training set.
...
We update $\pi$ via the Bayes rule according to her answer.
