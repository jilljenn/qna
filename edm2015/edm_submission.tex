...
\alert{A natural question is to ask whether 0-1 entries of the q-matrices should be replaced by real numbers. This approach was already proposed by Brewer~\cite{Barnes2005} (1996) and developed by SPARFA~\cite{Vats2013}, although their approach is more similar to MIRT. Convex optimization techniques could be used to efficient derive the q-matrix achieving minimal error. We plan to use this technique to see if we get better results.}
...
We devised a protocol to evaluate adaptive testing models, which allows us to test many more algorithms on large-scale datasets. Our simulation highlighted the role of slip and guess parameters compared to the item difficulty parameter of the Rasch model \alert{and reveals two possible directions for future research: test different regularization parameters linked to slip and guess in order to guarantee a fast recovery of the student skills, or examine fusion models that combine item response theory and diagnostic classification models}~\cite{McGlohen2008,Bradshaw2014}.
